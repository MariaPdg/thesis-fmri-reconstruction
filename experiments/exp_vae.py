...
if __name__ == "__main__":
    ...
    parser.add_argument('--mode', default='vae', help='vae, vae-gan, beta-vae, info-vae, dcgan')
    ...
    file_handler = logging.FileHandler(os.path.join(args.logs, 'train_exp_vae' + timestep))
    logger = logging.getLogger()
    ...
    if DEBUG:
        saving_dir = os.path.join(SAVE_PATH, 'debug', 'debug_exp_vae_{}'.format(timestep))
    else:
        saving_dir = os.path.join(SAVE_PATH, 'experiments', 'exp_vae_{}'.format(timestep))
    ...
    saving_name = os.path.join(saving_dir, 'exp_vae_{}.pth'.format(timestep))
