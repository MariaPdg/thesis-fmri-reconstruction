...
from models.vae_gan import WaeGan
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrained_gan', '-pretrain', default=wae_cfg.pretrained_gan, help='pretrained gan', type=str)
    parser.add_argument('-load_epoch', '-pretrain_epoch', default=wae_cfg.load_epoch, help='epoch of the pretrained model',
                        type=int)
    ...
    if DEBUG:
        saving_dir = os.path.join(SAVE_PATH, 'debug', 'debug_wae_gan_{}'.format(timestep))
    else:
        saving_dir = os.path.join(SAVE_PATH, 'wae_gan', 'wae_gan_{}'.format(timestep))
    if not os.path.exists(saving_dir):
        os.makedirs(saving_dir)
    if args.pretrained_gan is not None:
        pretrained_model_dir = os.path.join(SAVE_PATH, 'wae_gan', args.pretrained_gan, args.pretrained_gan + '.pth')
    saving_name = os.path.join(saving_dir, 'wae_gan_{}.pth'.format(timestep))
    ...
    training_data = CocoDataloader(COCO_TRAIN_DATA, pickle=False,
                                     transform=transforms.Compose([transforms.CenterCrop((args.image_crop,
                                                                   ...
                                                                   transforms.Normalize(wae_cfg.mean,
                                                                                        wae_cfg.std)
                                                                   ]))
    ...
    # Optimizers
    optimizer_encoder = torch.optim.Adam(model.encoder.parameters(), lr=args.learning_rate, betas=(0.5, 0.999))
    optimizer_decoder = torch.optim.Adam(model.decoder.parameters(), lr=args.learning_rate, betas=(0.5, 0.999))
    optimizer_discriminator = torch.optim.Adam(model.discriminator.parameters(), lr=0.5 * args.learning_rate,
                                               betas=(0.5, 0.999))

    lr_encoder = StepLR(optimizer_encoder, step_size=args.step_size, gamma=args.decay_lr)
    lr_decoder = StepLR(optimizer_decoder, step_size=args.step_size, gamma=args.decay_lr)
    lr_discriminator = StepLR(optimizer_discriminator, step_size=args.step_size, gamma=args.decay_lr)
