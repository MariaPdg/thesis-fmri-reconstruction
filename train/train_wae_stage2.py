...
from models.vae_gan import WaeGan, CognitiveEncoder, WaeGanCognitive
...
if __name__ == "__main__":
    ...
    file_handler = logging.FileHandler(os.path.join(args.logs, 'train_waegan_cog_' + timestep))
    logger = logging.getLogger()
    ...
    if DEBUG:
        saving_dir = os.path.join(SAVE_PATH, 'debug', 'debug_waegan_cog_{}'.format(timestep))
    else:
        saving_dir = os.path.join(SAVE_PATH, 'waegan_cog', 'waegan_cog_{}'.format(timestep))
    if not os.path.exists(saving_dir):
        os.makedirs(saving_dir)
    if args.pretrained_gan is not None:
        pretrained_model_dir = os.path.join(SAVE_PATH, 'waegan_cog', args.pretrained_gan, args.pretrained_gan + '.pth')
    saving_name = os.path.join(saving_dir, 'waegan_cog_{}.pth'.format(timestep))
    ...
    model = WaeGanCognitive(device=device, encoder=cognitive_encoder, decoder=trained_model.decoder, z_size=args.latent_dim,
                            recon_level=args.recon_level).to(device)
